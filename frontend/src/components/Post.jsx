...
const Post = ({ post }) => {
    ...
    const likeOrDislikeHandler = async () => {
        try {
            ...
            const res = await axios.get(` https://instavibe-ic2m.onrender.com//api/v1/post/${post._id}/${action}`, { withCredentials: true });
            if (res.data.success) {
    ...
    const commentHandler = async () => {
        ...
        try {
            const res = await axios.post(` https://instavibe-ic2m.onrender.com//api/v1/post/${post._id}/comment`, { text }, {
                headers: {
    ...
    const deletePostHandler = async () => {
        try {
            const res = await axios.delete(` https://instavibe-ic2m.onrender.com//api/v1/post/delete/${post?._id}`, { withCredentials: true })
            if (res.data.success) {
    ...
    const bookmarkHandler = async () => {
        try {
            const res = await axios.get(` https://instavibe-ic2m.onrender.com//api/v1/post/${post?._id}/bookmark`, { withCredentials: true });
            if (res.data.success) {
    ...
    return (
        <div className='my-8 w-full max-w-sm mx-auto'>
                ...
                <Dialog>
                    ...
                    <DialogContent className="flex flex-col items-center text-sm text-center">
                        {post?.author?._id !== user?._id && (
                            <Button
                                variant='ghost'
                                className={`cursor-pointer w-fit font-bold ${isFollowing ? 'text-[#ED4956]' : 'text-[#3BADF8]'}`}
                        ...
                        <Button variant='ghost' className="cursor-pointer w-fit" onClick={bookmarkHandler}>{isBookmarked?"Remove from favorites":"Add to favorites"}</Button>
                        {
            ...
            <div className='flex items-center justify-between my-2'>
                ...
                <Bookmark onClick={bookmarkHandler} className='cursor-pointer hover:text-gray-600' stroke="black"
